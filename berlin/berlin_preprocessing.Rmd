...
4. Religious Institutions (pre-processed in `berlin_osm_culture_religion.Rmd`)
5. Cultural Institutions (pre-processed in `berlin_osm_culture_religion.Rmd`)
...
- Data source: https://www.berlin.de/polizei/service/kriminalitaetsatlas/
```{r}
...
- Data source:
  - Miete per m2, 2019, fisbroker
  - Prognoseräume: fisbroker
```{r}
...
treecover_raw = raster(here("berlin", "raw_data", "treecover", "TCD_2018_010m_E45N32_03035_v020.tif"))

# Getting the CRS of the raster file: EPSG 3035
crs_3035 = crs(treecover_raw)
# Trannsforming the berlin geo to the crs
...
# Saving rds
# saveRDS(tree_mask, "berlin/preprocessed_data/berlin_tree_mask.rds")
# This has been done, but here the mask is loaded for plotting
tree_mask = readRDS(here("berlin", "preprocessed_data", "berlin_tree_mask.rds"))
# Plotting
plot(tree_mask)
plot(berlin_3035$geometry, add = T)
...
# Saving rds 
# saveRDS(tree_cropped, "berlin/preprocessed_data/berlin_tree_cropped.rds")
# This has been done, but here the mask is loaded for plotting
tree_cropped = readRDS(here("berlin", "preprocessed_data", "berlin_tree_cropped.rds"))
# Plotting
plot(tree_cropped)
plot(berlin_3035$geometry, add = T)
...
# Getting the CRS of the raster file: EPSG 3035
crs_3035 = crs(imperviousness_raw)
# Transforming the berlin geo to this crs
berlin_3035 = st_transform(berlin_geo, crs = crs_3035)

# Plotting the raster and geometry data
plot(imperviousness_raw)
plot(berlin_3035$geometry, add = T)

# Masking the raster data to only keep Berlin area
# imperv_mask = mask(imperviousness_raw, berlin_3035)
# Saving rds
# saveRDS(imperv_mask, "berlin/preprocessed_data/berlin_imperv_mask.rds")
# Read the rds for plotting 
imperv_mask = readRDS(here("berlin", "preprocessed_data", "berlin_imperv_mask.rds"))
# Plotting
plot(imperv_mask)
plot(berlin_3035$geometry, add = T)
...
# Saving rds
# saveRDS(imperv_cropped, "berlin/preprocessed_data/berlin_imperv_cropped.rds")
# Read the rds for plotting 
imperv_cropped = readRDS(here("berlin", "preprocessed_data", "berlin_imperv_cropped.rds"))
# Plotting
plot(imperv_cropped)
plot(berlin_3035$geometry, add = T)
